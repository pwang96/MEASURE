...
def convert_masscode(dirname):
    ...
    *******************************************************************************
    ...
    for x in range(nomstart + 1, nomend):
        ...
        nom = searchlines[x][15:].split()
        print [nom]
        mass_info['nominal'].append(float(nom[0]) * conversion)
        mass_info['density'].append(float(nom[1]))
        mass_info['coexpans'].append(float(nom[2]))
        try:
            mass_info['acceptcorrect'].append(float(nom[3]))
        except:
            mass_info['acceptcorrect'].append(0)

        volumecorrection = (1 + (volcoeffients[0] + 20 * volcoeffients[1]) / 1000000 * (20 - voltemp))
        try:
            mass_info['volume'].append(float(nom[4]) * volumecorrection)
        except:
    ...
    """
    f.close()
    a = MassCode(mass_info, testData, lists=True)

    json_path = ''
    try:
        json_path = dirname.replace('.ntxt', '')
    except:
        json_path = dirname
    ...
    original_info.update(updated_info)

    # Create the final, updated json file
    with open(json_path + '_final.json', 'w+') as final_file:
        json.dump(original_info, final_file)
        final_file.flush()

    # Remove the abridged json file
    os.remove(json_path + '_abridged.json')